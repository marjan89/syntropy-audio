...
function M.switch_device(device, device_type)
    ...
    -- Remove indicator prefix if present (icon + space)
    -- Uses %S+ to match multi-byte UTF-8 icons (all non-ASCII bytes are non-space)
    local clean_device = device:match("^%S+ (.+)$") or device
